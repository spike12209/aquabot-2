...
debug: ./src/**/*.cs
	...
	mcs --debug ./src/core/*.cs /target:library /out:./bin/aquaforms.dll \
		/r:System.Drawing \
		/r:./lib/atropos.dll \
		/r:./lib/Contest.Core.dll \
		/r:System.Windows.Forms
	
	mcs --debug ./src/*.cs /target:exe /out:./bin/aqua.exe \
		/r:System.Windows.Forms \
		/r:./lib/atropos.dll \
		/r:./bin/aquaforms.dll

	if [ -a ./src/tests/*.cs ]; then \
		mcs --debug ./src/tests/*.cs /target:library /out:./bin/aquatests.dll \
		/r:./lib/Contest.Core.dll \
		/r:./lib/atropos.dll; \
	fi;
test:
